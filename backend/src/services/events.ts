...
export const getEvent = async (eventId: string): Promise<Event | null> => {
  ...
  const inviteeRecords = await db.queryByPk<EventInviteeRecord>(eventPk(eventId), 'INVITEE#');
...
export const getEventsForUser = async (userId: string): Promise<Event[]> => {
  ...
  const hostedRecords = await db.queryByGsi1<EventRecord>(userPk(userId), 'EVENT#');
  
  // Get events user is invited to
  const inviteRecords = await db.queryByGsi1<EventInviteeRecord>(userPk(userId), 'INVITE#');
...
export const createEvent = async (
  ...
  const inviteeRecords: EventInviteeRecord[] = input.inviteeIds.map(userId => ({
    pk: eventPk(eventId),
    sk: inviteeSk(userId),
  ...
  return recordToEvent(eventRecord, invitees);
};
...
export const respondToEvent = async (
  ...
  const inviteeRecord = await db.getItem<EventInviteeRecord>(eventPk(eventId), inviteeSk(userId));
  if (!inviteeRecord) {
  ...
  await db.updateItem<EventInviteeRecord>(eventPk(eventId), inviteeSk(userId), updates);
...
const recordToEvent = (record: EventRecord, invitees: EventInvitee[]): Event => {
  return {
